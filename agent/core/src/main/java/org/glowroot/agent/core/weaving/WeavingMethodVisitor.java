...
import org.glowroot.agent.core.weaving.Advice.AdviceParameter;
import org.glowroot.agent.core.weaving.AdviceFlowOuterHolder.AdviceFlowHolder;
...
class WeavingMethodVisitor extends AdviceAdapter {
    ...
    private void loadOptionalReturnValue(int opcode, boolean dup) {
        if (opcode == RETURN) {
            // void
            visitMethodInsn(INVOKESTATIC, "org/glowroot/agent/core/weaving/VoidReturn",
                    "getInstance", "()Lorg/glowroot/agent/plugin/api/weaving/OptionalReturn;",
                    false);
        } else {
            loadReturnValue(opcode, dup, true);
            visitMethodInsn(INVOKESTATIC, "org/glowroot/agent/core/weaving/NonVoidReturn", "create",
    ...
    private void loadClassMeta(AdviceParameter parameter) {
        ...
        if (bootstrapClassLoader) {
            int index = BootstrapMetaHolders.reserveClassMetaHolderIndex(metaHolderInternalName,
                    classMetaFieldName);
            push(index);
            visitMethodInsn(INVOKESTATIC, "org/glowroot/agent/core/weaving/BootstrapMetaHolders",
