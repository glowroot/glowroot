...
public class Schemas {
    ...
    // JDBC type constants for type comparison (H2 2.x reports different TYPE_NAME strings
    // than H2 1.x, e.g. "CHARACTER VARYING" instead of "VARCHAR", but DATA_TYPE integers
    // from java.sql.Types are stable across H2 versions)
    private static final Map<ColumnType, Integer> jdbcTypes = Maps.newHashMap();

    static {
        // these are type mappings for H2 DDL
        ...
        typeNames.put(ColumnType.AUTO_IDENTITY, "bigint generated by default as identity");

        // JDBC type integers for type comparison in columnNamesAndTypesMatch()
        jdbcTypes.put(ColumnType.VARCHAR, Types.VARCHAR);
        jdbcTypes.put(ColumnType.BIGINT, Types.BIGINT);
        jdbcTypes.put(ColumnType.BOOLEAN, Types.BOOLEAN);
        jdbcTypes.put(ColumnType.VARBINARY, Types.VARBINARY);
        jdbcTypes.put(ColumnType.DOUBLE, Types.DOUBLE);
        jdbcTypes.put(ColumnType.AUTO_IDENTITY, Types.BIGINT); // auto-identity is a BIGINT column
    }
    ...
    private static boolean columnNamesAndTypesMatch(ResultSet resultSet,
        ...
        while (resultSet.next()) {
            ...
            Integer expectedJdbcType = jdbcTypes.get(column.type());
            if (expectedJdbcType == null) {
                return false;
            }
            // use DATA_TYPE (java.sql.Types integer) instead of TYPE_NAME string for comparison,
            // since H2 2.x reports different TYPE_NAME values than H2 1.x
            // (e.g. "CHARACTER VARYING" vs "VARCHAR") but DATA_TYPE integers are stable
            int actualJdbcType = resultSet.getInt("DATA_TYPE");
            if (expectedJdbcType != actualJdbcType) {
                return false;
            }
        }
