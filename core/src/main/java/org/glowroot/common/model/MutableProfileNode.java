...
public class MutableProfileNode extends AbstractCollection<MutableProfileNode>
    ...
    MutableProfileNode(@Nullable StackTraceElement stackTraceElement,
            @Nullable String leafThreadState) {
    ...
    // doesn't copy ellipsed sample count or the matched attribute
    //
    // should only be called on synthetic root node
    public MutableProfileNode copy() throws IOException {
        ProfileCopier copier = ProfileCopier.create(this);
        copier.traverse();
        return copier.syntheticRootNodeCopy;
    }
    ...
    private static class ProfileCopier extends Traverser<TraversePair, RuntimeException> {

        private final MutableProfileNode syntheticRootNodeCopy;

        private static ProfileCopier create(MutableProfileNode syntheticRootNode)
                throws IOException {
            TraversePair rootPair = ImmutableTraversePair.of(syntheticRootNode,
                    MutableProfileNode.createSyntheticRootNode());
            return new ProfileCopier(rootPair);
        }

        private ProfileCopier(TraversePair rootPair) {
            super(rootPair);
            syntheticRootNodeCopy = rootPair.copy();
        }

        @Override
        public Collection<? extends TraversePair> visit(TraversePair pair) {
            Collection<? extends MutableProfileNode> childNodes = pair.orig().childNodes();
            if (childNodes.isEmpty()) {
                return ImmutableList.of();
            }
            List<TraversePair> childPairs = Lists.newArrayList();
            for (MutableProfileNode childNode : childNodes) {
                // stackTraceElement() only returns null for synthetic root node
                StackTraceElement stackTraceElement = checkNotNull(childNode.stackTraceElement());
                MutableProfileNode childNodeCopy =
                        MutableProfileNode.create(stackTraceElement, childNode.leafThreadState());
                childNodeCopy.setSampleCount(childNode.sampleCount());
                childNodeCopy.setTimerNames(childNode.timerNames());
                // intentionally not copying ellipsed sample count or the matched attribute
                pair.copy().addChildNode(childNodeCopy);
                childPairs.add(ImmutableTraversePair.of(childNode, childNodeCopy));
            }
            return childPairs;
        }
    }
    ...
    @Value.Immutable
    @Styles.AllParameters
    static interface TraversePair {
        MutableProfileNode orig();
        MutableProfileNode copy();
    }
}
