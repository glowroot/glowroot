...
public class SummaryQuery {
    ...
    private final AggregateSortAttribute sortAttribute;
    private final SortDirection sortDirection;
    private final int limit;

    @VisibleForTesting
    SummaryQuery(String transactionType, long from, long to, AggregateSortAttribute sortAttribute,
    ...
    public AggregateSortAttribute getSortAttribute() {
    ...
    static SummaryQuery readValue(
            ...
            @JsonProperty("sortAttribute") @Nullable AggregateSortAttribute sortAttribute,
        ...
        return new SummaryQuery(transactionType, from, to, sortAttribute, sortDirection,
                limit);
    }

    @UsedByJsonBinding
    public static enum AggregateSortAttribute implements SortAttribute {

        TOTAL("sum(total_micros)"),
        AVERAGE("sum(total_micros) / sum(count)"),
        COUNT("sum(count)"),
        ERROR_COUNT("sum(error_count)"),
        TRACE_COUNT("sum(trace_count)");

        private final String column;

        private AggregateSortAttribute(String column) {
            this.column = column;
        }
