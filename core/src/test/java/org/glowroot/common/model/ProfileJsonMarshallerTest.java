...
public class ProfileJsonMarshallerTest {
    ...
    public void shouldWriteVeryLargeProfile() throws IOException {
        ...
        for (int i = 0; i < stackTrace.length; i++) {
            stackTrace[i] =
                    new StackTraceElement(ProfileJsonMarshallerTest.class.getName(), "method" + i,
                            ProfileJsonMarshallerTest.class.getName() + ".java", 100 + 10 * i);
        }
        profile.addToStackTree(Profile.stripSyntheticTimerMethods(Arrays.asList(stackTrace)),
                State.RUNNABLE.name());
        // when
        String profileJson = ProfileJsonMarshaller.marshal(profile.getSyntheticRootNode());
    ...
    public void shouldReadVeryLargeProfile() throws IOException {
        ...
        MutableProfileNode rootNode =
                ProfileJsonUnmarshaller.unmarshalProfile(new StringReader(profileJson));
        // then don't blow up with StackOverflowError
        // (and an extra verification just to make sure the test was valid)
