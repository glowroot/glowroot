...
public class RestTemplateAspect {
    ...
    @Pointcut(className = "org.springframework.web.client.RestTemplate", methodName = "doExecute",
            ...
            nestingGroup = "http-client", timerName = "http client resttemplate")
    public static class ExecuteAdvice {
        ...
        public static @Nullable TraceEntry onBefore(ThreadContext context,
                                                    @BindParameter URI uri, @BindParameter HttpMethod method) {
            String url = uri.toString();
            return context.startServiceCallEntry("HTTP", method + " " + Uris.stripQueryString(url),
                    MessageSupplier.from("http client resttemplate: {} {}", method.name(), url),
                    timerName);
        }
        @OnReturn
