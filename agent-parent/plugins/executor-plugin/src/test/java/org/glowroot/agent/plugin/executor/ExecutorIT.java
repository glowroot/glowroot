...
public class ExecutorIT {
    ...
    @Test
    public void shouldCaptureCallableAsync() throws Exception {
        // given
    ...
    @Test
    public void shouldCaptureRunnableAndCallableAsync() throws Exception {
        // given
        container.getConfigService().updateTransactionConfig(
                TransactionConfig.newBuilder()
                        .setProfilingIntervalMillis(OptionalInt.newBuilder().setValue(20).build())
    ...
    public static class DoSomeCallableAsyncWork implements AppUnderTest, TransactionMarker {
    ...
    public static class DoSomeRunnableAndCallableAsyncWork
        ...
        public void transactionMarker() throws Exception {
            ExecutorService executor = Executors.newCachedThreadPool();
            Future<Void> future1 =
                    executor.submit((Callable<Void>) new RunnableAndCallableAsyncWork());
            Future<Void> future2 =
                    executor.submit((Callable<Void>) new RunnableAndCallableAsyncWork());
            Future<Void> future3 =
                    executor.submit((Callable<Void>) new RunnableAndCallableAsyncWork());
    ...
    private static class RunnableAndCallableAsyncWork implements Runnable, Callable<Void> {
