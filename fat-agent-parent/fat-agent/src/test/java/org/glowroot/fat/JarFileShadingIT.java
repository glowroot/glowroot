...
public class JarFileShadingIT {
    ...
    public void shouldCheckThatJarIsWellShaded() throws IOException {
        File glowrootCoreJarFile = getGlowrootFatAgentJarFile();
    ...
    private static @Nullable File getGlowrootFatAgentJarFile() {
        for (File file : new File(".").listFiles()) {
            if (file.getName().matches("glowroot-fat-agent-core-[0-9.]+(-SNAPSHOT)?.jar")) {
                return file;
            }
        }
        return null;
