...
public class UiSandboxMain {
    ...
    public static void main(String... args) throws Exception {
        ...
        if (useJavaagent) {
            container = new JavaagentContainer(baseDir, false, false, ImmutableList.<String>of());
        } else {
            container = new LocalContainer(baseDir, false, ImmutableMap.<String, String>of());
        }
        if (initConfig) {
            container.getConfigService().updateTransactionConfig(
                    TransactionConfigUpdate.newBuilder()
                            .setProfilingIntervalMillis(OptionalInt.newBuilder().setValue(100))
                            .build());
        }
