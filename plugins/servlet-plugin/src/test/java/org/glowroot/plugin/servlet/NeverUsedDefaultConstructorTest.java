...
//
// also see copies of this in glowroot-core, jdbc-plugin and logger-plugin
public class NeverUsedDefaultConstructorTest {
    ...
    public void testNeverUsedDefaultConstructors() throws Exception {
        ...
        for (ClassInfo classInfo : classInfos) {
            if (!classInfo.getName().startsWith("org.glowroot.plugin.servlet")) {
                continue;
            }
            Class<?> servletPluginClass = classInfo.load();
            try {
                testDefaultConstructorIfPointcutAdviceClass(servletPluginClass);
            } catch (Exception e) {
            }
            try {
                testPrivateDefaultConstructorIfUtilityClass(servletPluginClass);
            } catch (Exception e) {
            }
        }
    }
